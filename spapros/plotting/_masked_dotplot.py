...
class MaskedDotPlot(sc.pl.DotPlot):
    ...
    def __init__(
        ...
        **kwds,
        ...
        default_length = (self.DEFAULT_CATEGORY_WIDTH + self.DEFAULT_CATEGORY_HEIGHT)/2
    ...
    def _dotplot(
        ...
        **kwds,
        ...
        if color_on == 'square':
            ...
            if isinstance(tree_genes,pd.DataFrame):            
                dot_ax.scatter(x[tree_genes_mask], 
                               y[tree_genes_mask], 
                               s=np.max(size)*1.55, 
                               marker='s', 
                               color='none',
            ...
            if isinstance(grey_celltypes,pd.DataFrame):
                dot_ax.scatter(x[grey_celltypes_mask], 
            ...
            if isinstance(marker_genes,pd.DataFrame):   
                dot_ax.scatter(x[marker_genes_mask],
                               y[marker_genes_mask],
                               s=np.max(size)*1.85,
                               marker='s',
                               color='green',
    ...
    def _plot_legend(self, legend_ax, return_ax_dict, normalize):
        ...
        height_ratios = [
            ...
            self.height - 3*marker_legend_height - size_legend_height - cbar_legend_height - 2*spacer_height\
        ...
        fig, legend_gs = make_grid_spec(
            legend_ax, nrows=10, ncols=1, height_ratios=height_ratios
        )
