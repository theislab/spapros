...
def spapros_questionary(function: str, question: str, choices: list = None, default: str = None) -> Union[str, bool]:
    """
    ...
    try:
        if function == "select":
            ...
            if not default:
                ...
                default = ""
            answer = getattr(questionary, function)(f"{question} [{default}]: ", style=spapros_style).unsafe_ask()
        elif function == "confirm":
            default_value_bool = True if default == "Yes" or default == "yes" else False
            answer = getattr(questionary, function)(
                f"{question} [{default}]: ", style=spapros_style, default=default_value_bool
            ).unsafe_ask()
        else:
